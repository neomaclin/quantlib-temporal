...
final case class ECB[D: DateOps](dates: List[Int]) {

  def isECBdate(date: D): Boolean = ???

  def isECBcode(code: String): Boolean = ???

  def addDate(date: D): ECB[D] = ???

  def removeDate(date: D): ECB[D] = ???

  def nextDate(code: String): D = ???

  def nextDates(code: String): List[D] = ???

  def nextDate(date: D): D =  ???

  def nextDates(date: D): List[D] =  ???

  def code(date: D): String =  ???

  def date(code: String): D = ???

//  def nextCode(date: D = Settings.evaluationDate): String = ???
